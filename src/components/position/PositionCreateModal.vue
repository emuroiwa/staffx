  ...
  <div class="fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-80 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] flex flex-col">
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add New Position</h3>
        <button
          @click="$emit('close')"
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div>
            <label class="flex items-center">
              ...
              />
              <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Active Position</span>
            </label>
          </div>
        </form>
      </div>
      ...
      <div class="flex items-center justify-end p-6 border-t border-gray-200 dark:border-gray-700 space-x-3">
        ...
        <button
          ...
          type="button"
          :disabled="submitting"
          class="inline-flex items-center px-4 py-2 bg-blue-600 dark:bg-blue-500 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-blue-700 dark:hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
        >
...
const handleSubmit = async () => {
  ...
  try {
    await employeeStore.createPosition(form)
    notificationStore.addNotification({
      type: 'success',
      title: 'Success',
